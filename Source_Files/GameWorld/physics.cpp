...
typedef struct physics_constants;
physics_constants *get_physics_constants_for_model(short physics_model, uint32 action_flags);
void instantiate_physics_variables(struct physics_constants *constants, struct physics_variables *variables, short player_index, bool first_time, bool take_action);
...
void initialize_player_physics_variables(short player_index)
{
	...
	physics_constants *constants= get_physics_constants_for_model(static_world->physics_model, 0);
...
void update_player_physics_variables(short player_index, uint32 action_flags, bool predictive)
{
	player_data *player = get_player_data(player_index);
	physics_variables *variables = &player->variables;
	physics_constants *constants = get_physics_constants_for_model(static_world->physics_model, action_flags);
	...
	{
		object_data *object= get_object_data(get_monster_data(player->monster_index)->object_index);
...
void adjust_player_for_polygon_height_change(short monster_index, short polygon_index, world_distance new_floor_height, world_distance new_ceiling_height)
{
...
{
	player_data_from_monster_index(monster_index)->accelerate(vertical_velocity, direction, velocity);
}
...
uint32 mask_in_absolute_positioning_information(uint32 action_flags, _fixed delta_yaw, _fixed delta_pitch, _fixed delta_position)
{
	...
	if ((delta_yaw||variables->angular_velocity) && !(action_flags&_override_absolute_yaw))
	{
		...
		if (delta_yaw>0 && delta_yaw<((1)<<(FIXED_FRACTIONAL_BITS-ABSOLUTE_YAW_BITS))) 
...
void get_binocular_vision_origins(short player_index, world_point3d *left, short *left_polygon_index, angle *left_angle,
	world_point3d *right, short *right_polygon_index, angle *right_angle)
{
	player_data *player = get_player_data(player_index);
	physics_variables *variables = &player->variables;
	const physics_constants *constants = get_physics_constants_for_model(static_world->physics_model, 0);

	auto theta = NORMALIZE_ANGLE(player->facing + QUARTER_CIRCLE);
...
{
	return get_player_data(player_index)->get_forward_velocity_scale();
}
...
physics_constants *get_physics_constants_for_model(short physics_model, uint32 action_flags)
{
	assert(physics_model == _earth_gravity_model || physics_model == _editor_model);
	return physics_models + (action_flags & _run_dont_walk ? _model_game_running : _model_game_walking);
}
...
static void physics_update(struct physics_constants *constants, struct physics_variables *variables, struct player_data *player, uint32 action_flags)
{
	...
	if (PLAYER_IS_DEAD(player)) /* dead players immediately loose all bodily control */
	{
		...
		auto dot_product = ((((variables->velocity*cosine) >> TRIG_SHIFT) + variables->external_velocity.i) * cosine +
			(((variables->velocity * sine) >> TRIG_SHIFT) + variables->external_velocity.j)*sine)>>TRIG_SHIFT;

		if (dot_product > 0 && dot_product < constants->maximum_forward_velocity / 16 ) 
			dot_product = 0;
		switch (SGN(dot_product))
		{
			case -1: 
				...
				break;
			case 1: 
				action_flags= _looking_down; break;
			case 0: 
				action_flags = 0; break;
			default:
				assert(false);
				break;
		}
	...
	{
		if (variables->angular_velocity<-constants->maximum_angular_velocity||variables->angular_velocity>constants->maximum_angular_velocity) 
			action_flags&= ~_turning;
	...
	if (delta_z <= 0 || variables->flags & _HEAD_BELOW_MEDIA_BIT)
	{
		...
		{
			auto encoded_delta = GET_ABSOLUTE_POSITION(action_flags)-MAXIMUM_ABSOLUTE_POSITION/2;
	...
	{
		auto gravity 			= constants->gravitational_acceleration;
		auto terminal_velocity 	= constants->terminal_velocity;
	...
	if (variables->external_velocity.k>0 && new_position.z+variables->actual_height>=variables->ceiling_height)
		variables->external_velocity.k/= -COEFFICIENT_OF_ABSORBTION, new_position.z= variables->ceiling_height-variables->actual_height;
	...
	if (player->get_player_monster()->getDefinition()->flags & _monster_can_grenade_climb) 
	{
		auto gravity = constants->gravitational_acceleration;		
	...
	{
		auto dx = variables->external_velocity.i, dy= variables->external_velocity.j;
		auto delta = (delta_z<=0) ? constants->external_deceleration : (constants->external_deceleration>>2);
