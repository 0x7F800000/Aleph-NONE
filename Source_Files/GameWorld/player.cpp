...
static void give_player_initial_items(short player_index)
{
	get_player_data(player_index)->give_initial_items();
}
...
player_data* player_data_from_monster_index(uint16 monster_index)
{
	return get_player_data(monster_index_to_player_index(monster_index));
}

void player_data::accelerate(world_distance vertical_velocity, angle direction, world_distance velocity)
{
	static const int shiftfactor = TRIG_SHIFT + WORLD_FRACTIONAL_BITS - FIXED_FRACTIONAL_BITS;
	
	_fixed terminal_velocity = get_physics_constants_for_model(static_world->physics_model, 0)->terminal_velocity;
	
	variables.external_velocity.k = 
	PIN(variables.external_velocity.k + WORLD_TO_FIXED(vertical_velocity), -terminal_velocity, terminal_velocity);
	
	if(	get_player_monster_definition()->flags & _monster_can_grenade_climb)
	//get_monster_definition_external(_monster_marine)->flags & _monster_can_grenade_climb)
	{
		variables.external_velocity.i = (	cosine_table[	direction	] * velocity ) >> shiftfactor;
		variables.external_velocity.j = (	sine_table[		direction	] * velocity ) >> shiftfactor;
		return;
	}
	variables.external_velocity.i += (	cosine_table[	direction	] * velocity ) >> shiftfactor;
	variables.external_velocity.j += (	sine_table[		direction	] * velocity ) >> shiftfactor;
...
bool player_data::isTeleporting()
{
	return PLAYER_IS_TELEPORTING(this);
}

bool player_data::isInterlevelTeleporting()
{
	return PLAYER_IS_INTERLEVEL_TELEPORTING(this);
}

void player_data::get_transfer_mode(int16* transfer_mode, int16* transfer_period)
{
	int duration = 0;
	...
	*transfer_mode = NONE;
	...
	if(isTeleporting())
	{
		if(teleporting_destination >= 0 || (teleporting_phase < PLAYER_TELEPORTING_MIDPOINT && View_DoInterlevelTeleportOutEffects()) || (teleporting_phase >= PLAYER_TELEPORTING_MIDPOINT && View_DoInterlevelTeleportInEffects()))
			goto SetTeleportingTransferMode;
		return;
	}
	else if(isInterlevelTeleporting())
	{
		if(teleporting_destination >= 0 || (teleporting_phase < PLAYER_TELEPORTING_MIDPOINT && View_DoInterlevelTeleportOutEffects()) || (teleporting_phase >= PLAYER_TELEPORTING_MIDPOINT && View_DoInterlevelTeleportInEffects()))
		{
SetTeleportingTransferMode:
			*transfer_mode = teleporting_phase < PLAYER_TELEPORTING_MIDPOINT ? _xfer_fold_out : _xfer_fold_in;
	...
	else if(invisibility_duration)
	{
		*transfer_mode = invisibility_duration > kINVISIBILITY_DURATION ? _xfer_subtle_invisibility : _xfer_invisibility;
	...
	switch(duration / (TICKS_PER_SECOND / 6) )
	{
		case 46: case 37: case 29: case 22: case 16: case 11: case 7: case 4: case 2:
			if(invincibility_duration && invisibility_duration)
				*transfer_mode = invisibility_duration > kINVISIBILITY_DURATION ? _xfer_subtle_invisibility : _xfer_invisibility;
			else	
				*transfer_mode = NONE;
			break;
...
int16 player_data::get_player_index()
{
	return player_identifier_to_player_index(identifier);
}

void player_data::give_initial_items()
{
	for(int i = 0; i < NUMBER_OF_PLAYER_INITIAL_ITEMS; ++i)
	{
		int16 count = items[	player_initial_items[i]	];
		items[	player_initial_items[i]	] = count == NONE ? 1 : count + 1;
		process_new_item_for_reloading(get_player_index(), player_initial_items[i]);
	}
}
...
struct monster_definition* player_data::get_player_monster_definition()
{
	return get_player_monster()->getDefinition();
}

_fixed player_data::get_forward_velocity_scale()
{
	physics_constants *constants = get_physics_constants_for_model(static_world->physics_model, _run_dont_walk);
	
	_fixed dx = variables.position.x - variables.last_position.x;
	_fixed dy = variables.position.y - variables.last_position.y;

	return INTEGER_TO_FIXED((
	(	dx	*	cosine_table[	FIXED_INTEGERAL_PART(variables.direction)	] 
		+	dy	*	sine_table[	FIXED_INTEGERAL_PART(variables.direction)	])	>> TRIG_SHIFT))	
