...
struct monster_data* monster_data::Get(const ix index)
{
...
short new_monster(struct object_location *location, short monster_type)
{
	...
	if( !definition->testFlags(_monster_cannot_be_dropped | _monster_is_alien) && not isNONE(drop_mask) && 
	...
	if(definition->testFlags(_monster_major))
	{
		...
		if(not isNONE(demote_mask) and not (++dynamic_world->new_monster_mangler_cookie & demote_mask)) 
		{
	...
	else if(definition->testFlags(_monster_minor))
	{
		...
		if( not isNONE(promote_mask) && !(++dynamic_world->new_monster_mangler_cookie & promote_mask)) 
		{
			definition= get_monster_definition(++monster_type);
	...
	auto f = [=, &monster_index, &definition, &flags, &location, monster_type](monster_data &m)
	{
		...
		object_data* obj = get_object_data( object_index );
				...
				switch (monster->action)
				{
					...
					case _monster_is_dying_flaming:
						...
						if(monster->testDefinitionFlags(_monster_has_delayed_hard_death) && monster->isDyingSoft() )
						{
							...
							else if((contrail = monster-> getDefinition()->contrail_effect) != NONE) 
...
void activate_nearby_monsters(
	...
	int32 max_range)
{
	monster_data *caller = get_monster_data(caller_index);
	...
	{
		auto polygon_index = get_object_data(caller->object_index)->polygon;
		...
		while(	!isNONE(polygon_index)	)
		{
			...
			for (object_index = get_polygon_data(polygon_index)->first_object; object_index != NONE; object_index= object->next_object)
			{
				...
				if(GET_OBJECT_OWNER(object) == _object_is_monster && (!object->isInvisible() || flags & _activate_invisible_monsters) )
				{
					...
					{
						bool monster_was_active;
						/* activate the monster if he’s inactive */
						if(! ( monster_was_active = aggressor->isActive() ) )
...
static int32 monster_activation_flood_proc(short source_polygon_index, short line_index, short destination_polygon_index, void *data)
{
...
bool live_aliens_on_map()
{
	...
	foreach_monster(monster_index, monster)
	{
		if( !monster->slotIsUsed() )
			continue;
		if( (found_alien_which_must_be_killed = monster->mustBeExterminated() ) )
			break;
...
void activate_monster(short monster_index)
{
	...
	assert( not monster->isActive() );
	assert( not monster->isPlayer() );

	if( object->isInvisible() )
	{
		polygon_data *polygon = get_polygon_data(object->polygon);
		
		if( not isNONE(polygon->media_index) )
		{
			media_data *media = get_media_data(polygon->media_index);
			if (media && media->height > object->location.z + definition->height && not definition->testFlags(_monster_can_teleport_under_media) )
					return;
		}
	}
	...
	definition->testFlags(_monster_attacks_immediately) 
	?	definition->attack_frequency 
...
void deactivate_monster(short monster_index)
{
	...
	if( monster->teleportsOutWhenDeactivated() && not monster->isTeleportingOut() )
			...
			while (object_index!=NONE)
			{
				...
				if (!OBJECT_IS_INVISIBLE(object))
				{
					switch (GET_OBJECT_OWNER(object))
					{
						...
						{
							monster_data *monster= get_monster_data(object->permutation);
...
void monster_moved(short target_index, short old_polygon_index)
{
	...
	for (monster_index=0,monster=monsters;monster_index<MAXIMUM_MONSTERS_PER_MAP;++monster_index,++monster)
	{
		...
		if (monster->slotIsUsed() && monster->hasValidTarget() && monster->isTarget(target_index))
		{
			...
			{
				monster_definition *definition = monster->getDefinition();//monster->getDefinition();
				auto intelligence = definition->intelligence;
				
				switch (dynamic_world->game_information.difficulty_level)
				{
					case _wuss_level: 
						intelligence = intelligence >> 2; break;
...
short legal_player_move(short monster_index, world_point3d *new_location, world_distance *object_floor) /* must be set on entry */
{
	monster_data *monster = get_monster_data(monster_index);
	object_data *object = get_object_data(monster->object_index);
	...
	{
		object_data *obstacle = get_object_data(IntersectedObjects[i]);
	...
	struct damage_definition *damage, short projectile_index)
{
	monster_data *monster = get_monster_data(target_index);
	monster_definition *definition = monster->getDefinition();
	monster_data *aggressor_monster = aggressor_index != NONE ? monster_data::Get(aggressor_index) : nullptr;//(monster_data *) NULL;
	...
	if (!(definition->immunities & FLAG(damage->type)))
	{
		...
		else
		{
			...
			if( not isNONE(aggressor_index) && aggressor_monster->isPlayer() )
			{
		...
		if(external_velocity and epicenter)
		{
			object_data *object = get_object_data(monster->object_index);
			...
			auto vertical_velocity= (vertical_component&&radius) ? ((external_velocity*dz)/radius) : 0;
...
bool legal_polygon_height_change(short polygon_index, world_distance new_floor_height, world_distance new_ceiling_height,
	struct damage_definition *damage)
{
...
void accelerate_monster(short monster_index,
	...
	world_distance velocity)
{
	...
	PIN(monster->vertical_velocity + vertical_velocity, 
...
static void update_monster_vertical_physics_model(short monster_index)
{
	...
	media_data *media = polygon->media_index == NONE ? nullptr : get_media_data(polygon->media_index);
	auto moving_flags = MONSTER_IS_DYING(monster) ? 0 : (definition->flags&(_monster_flys|_monster_floats));
	
	auto gravity = (static_world->environment_flags&_environment_low_gravity) ? (definition->gravity>>1) : definition->gravity;
	...
	if (moving_flags&_monster_flys)
	{
		...
		if (monster->hasValidTarget())
		{
			...
			monster_definition *target_definition = target->getDefinition();//get_monster_definition(target->type);
	...
	short delta_vitality)
{
	...
	{
		if (monster->hasValidTarget() && monster->target_index==attacker_index)
		{
		...
		{
			monster_definition *definition= monster->getDefinition();
			short target_index= monster->hasValidTarget() ? monster->target_index : NONE;
			monster_data *attacker= get_monster_data(attacker_index);
...
static short get_monster_attitude(short monster_index, short target_index)
{
	monster_data *monster = get_monster_data(monster_index);
	monster_definition *definition = monster->getDefinition();
	monster_data *target = get_monster_data(target_index);
	...
	if (mTYPE_IS_ENEMY(definition, target_type) || MONSTER_IS_BERSERK(monster) ||
		(monster->hasValidTarget() && monster->target_index==target_index) ||
...
{
	monster_data *aggressor= get_monster_data(aggressor_index);
	monster_definition *definition= aggressor->getDefinition();//get_monster_definition(aggressor->type);
		...
		while (polygon_index!=NONE && closest_hostile_target_index==NONE)
		{
			...
			for (object_index= get_polygon_data(polygon_index)->first_object; object_index!=NONE; object_index= object->next_object)
			{
				...
				if (GET_OBJECT_OWNER(object)==_object_is_monster && OBJECT_IS_VISIBLE(object))
				{
					...
					monster_data *target_monster= get_monster_data(target_monster_index);
...
static bool clear_line_of_sight(short viewer_index, short target_index, bool full_circle)
{
	monster_data *viewer 				= get_monster_data(viewer_index);
	object_data *viewer_object			= get_object_data(viewer->object_index);
	monster_definition *viewer_definition
...
void change_monster_target(short monster_index, short target_index)
{
	...
	if (monster->target_index != target_index and TYPE_IS_FRIEND(monster->getDefinition(), get_monster_data(target_index)->type))
		play_object_sound(monster->object_index, monster->getDefinition()->friendly_activation_sound);
		
	else if(not monster->testDefinitionFlags(_monster_makes_sound_when_activated) and monster->isUnlocked() ) 
	...
	if (monster->hasValidTarget() and target_index != monster->getTarget()) 
		CLEAR_TARGET_DAMAGE_FLAG(monster);
	...
	else
	{
		...
		if (GET_OBJECT_OWNER(obstacle_object)==_object_is_monster)
		{
			...
			if (!mTYPE_IS_ENEMY(definition, obstacle_monster->type) && !( monster->hasValidTarget() && monster->isTarget(obstacle_object->permutation) ) &&
				!MONSTER_IS_BERSERK(monster))
			{
				if (not obstacle_monster->isPlayer())
				{
			...
			else
			{
				...
				auto key_height = obstacle_object->location.z + ( obstacle_definition->height >> 1 );
...
int32 monster_pathfinding_cost_function(short source_polygon_index, short line_index, short destination_polygon_index, void *vdata)
{
	monster_pathfinding_data *data = (monster_pathfinding_data *)vdata;
	monster_definition *definition = data->definition;
	polygon_data *destination_polygon = get_polygon_data(destination_polygon_index);
	polygon_data *source_polygon = get_polygon_data(source_polygon_index);
	line_data *line = get_line_data(line_index);
			...
			default:
				...
				if (!(definition->flags&(_monster_floats|_monster_flys)) && polygon->media_index!=NONE)
				{
					...
					if (media)
					{
						...
						switch (media->type)
						{
							case _media_water: if (definition->flags&_monster_is_not_afraid_of_water) media= (media_data *) NULL; break;
							case _media_jjaro: // LP addition: monsters treat Jjaro goo like sewage
							case _media_sewage: if (definition->flags&_monster_is_not_afraid_of_sewage) media= (media_data *) NULL; break;
							case _media_lava: /* height= 0; */ if (definition->flags&_monster_is_not_afraid_of_lava) media= (media_data *) NULL; break;
							case _media_goo: /* height= 0; */ if (definition->flags&_monster_is_not_afraid_of_goo) media= (media_data *) NULL; break;
						}
					}
	...
	angle theta)
{
	monster_data *aggressor = get_monster_data(aggressor_index);
	monster_data *target = get_monster_data(target_index);
	object_data *aggressor_object = get_object_data(aggressor->object_index);
	object_data *target_object = get_object_data(target->object_index);
	world_distance radius, height;
...
short nearest_goal_polygon_index(short polygon_index)
{
	...
	while(not isNONE(polygon_index))
	{
		polygon_data *polygon = get_polygon_data(polygon_index);
...
static int32 nearest_goal_cost_function(short source_polygon_index, short line_index, short destination_polygon_index, void *unused)
{
	polygon_data *destination_polygon = get_polygon_data(destination_polygon_index);
	auto cost = 1;
...
struct monster_definition*	monster_data::getDefinition()
{
...
bool monster_data::testDefinitionFlags(uint32 flagtest)	
{	
	return  getDefinition()->flags & flagtest;	
}

bool monster_data::canFlyOrFloat()	
{	
	return testDefinitionFlags(_monster_floats|_monster_flys);	
}

struct object_data* monster_data::getObject()
{
	return get_object_data(object_index);
}

bool monster_data::testObjectFlags(uint16 flagtest)
{
	return getObject()->flags & flagtest;
}

bool monster_data::testAnimationFlags(uint16 flagtest)
{
	return GET_OBJECT_ANIMATION_FLAGS(getObject()) & flagtest;
}

void monster_data::removePath()
{
	if(	hasPath()	) 
		delete_path( getPath() );
...
bool monster_data::mustBeExterminated()
{
	return monster_must_be_exterminated[type];
}

bool monster_data::slotIsUsed()
{
	return SLOT_IS_USED(this);
}

bool monster_data::slotIsFree()
{
