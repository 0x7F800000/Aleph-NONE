...
void move_monsters()
{
	...
	foreach_monster(monster_index, monster)
	{
		...
		if(	monster->isActive()	)
		{
			...
			/* update our object's animation unless we're Ôsuffering' from an external velocity
...
short legal_monster_move(
	short monster_index,
	angle facing, /* could be different than object->facing for players and Ôflying' (heh heh) monsters */
...
uint32 monster_data::testDefinitionFlags(uint32 flagtest)	
{	
...
uint16 monster_data::testObjectFlags(uint16 flagtest)
{
	return getObject()->flags & flagtest;
}

uint16 monster_data::testAnimationFlags(uint16 flagtest)
{
