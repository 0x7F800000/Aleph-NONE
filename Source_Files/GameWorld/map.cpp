...
{
	decltype(dynamic_world->total_civilian_causalties) total_civilians, total_causalties;
	
	decltype(dynamic_world->tick_count) 		tick_count;
	decltype(dynamic_world->random_seed) 		random_seed;
	decltype(dynamic_world->player_count) 		player_count;
	decltype(dynamic_world->game_information) 	game_information;
...
void mark_map_collections(bool loading)
{
	...
	for(ix n = 0; n < dynamic_world->polygon_count; n++)
	{
		polygon_data &polygon = polygon_data::Get(n);	//map_polygons + n;
		auto coll = GET_DESCRIPTOR_COLLECTION(polygon.floor_texture);
...
short find_new_object_polygon(world_point2d *parent_location, world_point2d *child_location, short parent_polygon_index)
{
	...
	while ( !isNONE( line_index ) && !isNONE( child_polygon_index ) );
...
{
	object_data &host = object_data::Get(host_index);
	object_data &parasite = object_data::Get(host.parasitic_object);
...
void remove_map_object(short object_index)
{
	...
	{
		object_data *parasite= get_object_data(object->parasitic_object);
...
void perform_deferred_polygon_object_list_manipulations()
{
	...
	while(!sDeferredObjectListInsertions.empty())
	{
		...
		{
			auto object_to_insert_index = (*i).first;
			auto object_index_to_precede = (*i).second;
...
{
	const line_data &line = line_data::Get(line_index);
	
	const world_point2d &e0 = endpoint_data::Get( line.endpoint_indexes[ 0 ] ).vertex;
	const world_point2d &e1 = endpoint_data::Get( line.endpoint_indexes[ 1 ] ).vertex;
...
{
	line_data &line = line_data::Get(line_index);
	decltype(polygon_index) new_polygon_index;
	
	if (polygon_index == line.clockwise_polygon_owner)
		new_polygon_index = line.counterclockwise_polygon_owner;
	else	// LP change: get around some Pfhorte bugs
...
short find_flooding_polygon(short polygon_index)
{
	...
	for( ix i = 0; i < polygon.vertex_count; ++i )
	{
		if ( isNONE( polygon.adjacent_polygon_indexes[ i ] ) )
			continue;
		
		polygon_data &adjacent_polygon = polygon_data::Get( polygon.adjacent_polygon_indexes[i] );
		
		if (adjacent_polygon.type == _polygon_is_major_ouch || adjacent_polygon.type == _polygon_is_minor_ouch)
...
short find_adjacent_side(short polygon_index, short line_index)
{
	...
	decltype(polygon_index) side_index;
...
bool line_is_landscaped(short polygon_index, short line_index, world_distance z)
{
	...
	line_data &line = line_data::Get(line_index);
	side_data &side = side_data::Get(side_index);
...
{
	polygon_data &polygon = polygon_data::Get(polygon_index1);
	short shared_line_index = NONE;
	for ( ix i = 0; i < polygon.vertex_count && isNONE( shared_line_index ); ++i )
	{
		line_data &line = line_data::Get( polygon.line_indexes[ i ] );
...
_fixed closest_point_on_line(world_point2d *e0, world_point2d * e1, world_point2d *p, world_point2d *closest_point)
{
	...
	decltype(p->x) dx = p->x - e0->x;
	decltype(p->y) dy = p->y - e0->y;
	
	decltype(e1->x) line_dx = e1->x - e0->x;
	decltype(e1->y) line_dy = e1->y - e0->y;
	
	/* same comment as above for calculating t; this is not wholly accurate */
	auto numerator		= line_dx * dx + line_dy * dy;
	auto denominator	= line_dx * line_dx + line_dy * line_dy;
	...
	auto isZeroOrAbsOne = [](decltype(calculated_closest_point.y-p->y) val)
	{
		return val == -1 || !val || val == 1;
	};
	
	if(isZeroOrAbsOne(calculated_closest_point.x-p->x) && isZeroOrAbsOne(calculated_closest_point.y-p->y))
...
{
	decltype(p->x) dx = p->x - c->x;
	decltype(p->y) dy = p->y - c->y;
	auto magnitude = isqrt( SQUARE(dx) + SQUARE(dy) );
...
void find_center_of_polygon(short polygon_index, world_point2d *center)
{
	...
	for(ix i = 0; i < polygon->vertex_count; ++i)
	{
		const world_point2d *p = &get_endpoint_data(	polygon->endpoint_indexes[i]	)->vertex;
		x += p->x;
		y += p->y;
	}
	...
	short *supporting_polygon_index)
{
	...
	*adjusted_ceiling_height= polygon->ceiling_height;
	do
	{
		...
		{
			auto signed_line_index = indexes[i];
			auto unsigned_line_index= signed_line_index<0 ? -signed_line_index-1 : signed_line_index;
			
			// If there is some map-index screwup...
			if (unsigned_line_index >= dynamic_world->line_count)
				continue;
			
			line_data *line= get_line_data(unsigned_line_index);
			auto side_index= signed_line_index<0 ? line->counterclockwise_polygon_side_index : line->clockwise_polygon_side_index;
				
			if (side_index == NONE)
				continue;
			
			side_exclusion_zone *zone= &get_side_data(side_index)->exclusion_zone;
			...
			{
				auto adjacent_polygon_index= signed_line_index<0 ? line->clockwise_polygon_owner : line->counterclockwise_polygon_owner;
				polygon_data *adjacent_polygon= adjacent_polygon_index==NONE ? nullptr : get_polygon_data(adjacent_polygon_index);
	...
	if (state!=_aborted)
	{
		...
		{
			auto endpoint_index = indexes[polygon->line_exclusion_zone_count+i];
			...
			auto dx = endpoint->vertex.x - p1->x;
			auto dy = endpoint->vertex.y - p1->y;
			auto distance_squared= SQUARE(dx) + SQUARE(dy);
...
{
	auto dx = cosine_table[theta];
	auto dy = sine_table[theta];
	auto x = (int32)p0->x + (int32)((d*dx)>>TRIG_SHIFT);
	auto y = (int32)p0->y + (int32)((d*dy)>>TRIG_SHIFT);
...
{
	map_annotation *annotation = nullptr;
...
struct guard_path_header *get_guard_path_header(short guard_path_index)
{
...
bool change_polygon_height(short polygon_index, world_distance new_floor_height, world_distance new_ceiling_height, struct damage_definition *damage)
{
	...
	bool legal_change = legal_polygon_height_change(polygon_index, new_floor_height, new_ceiling_height, damage);
...
bool point_is_player_visible(short max_players, short polygon_index, world_point2d *p, int32 *distance)
{
	...
	*distance = INT32_MAX; /* infinite */
	for(ix player_index = 0; player_index < max_players; ++player_index)
	{
		...
		{
			auto this_distance = guess_distance2d((world_point2d*)&object->location, p);
...
bool point_is_monster_visible(short polygon_index, world_point2d *p, int32 *distance)
{
	...
	*distance = INT32_MAX; /* infinite */
	...
	{
		// LP change:
		object_data *object= get_object_data(IntersectedObjects[i]);
		...
		else
		{
			...
			if (polygon_index != polygon_index2) 
			{
				...
				if (film_profile.line_is_obstructed_fix)
				{
					...
					for (ix i = 0; i < polygon->vertex_count; ++i)
					{
						for (ix j = 0; j < polygon2->vertex_count; ++j)
						{
	  ...
	  if (dynamic_world->garbage_object_count>= (graphics_preferences->double_corpse_limit? MAXIMUM_GARBAGE_OBJECTS_PER_MAP * 2 : MAXIMUM_GARBAGE_OBJECTS_PER_MAP))
	    {
			...
			dynamic_world->garbage_object_count++;
	}
...
/* find an (x,y) and polygon_index for a random point on the given circle, at the same height
	as the center point */
void random_point_on_circle(world_point3d *center, short center_polygon_index, world_distance radius, world_point3d *random_point,
short *random_polygon_index)
{
	...
	*random_polygon_index= find_new_object_polygon((world_point2d *)center,
	...
	{
		polygon_data *center_polygon = get_polygon_data(center_polygon_index);
		polygon_data *random_polygon = get_polygon_data(*random_polygon_index);
...
bool line_has_variable_height(short line_index)
{
	...
	return polygon->type == _polygon_is_platform;
}
...
{
	return get_game_state()==_game_in_progress ? (world_location3d*)&current_player->camera_location : nullptr;
}
...
uint16 _sound_obstructed_proc(world_location3d *source)
{
	...
	bool source_under_media = false;
	bool listener_under_media = false;
	...
	if(source_polygon->media_index != NONE)
	{
		...
		if(media && source->point.z < media->height)
...
void _sound_add_ambient_sources_proc(void *data, add_ambient_sound_source_proc_ptr add_one_ambient_sound_source)
{
	...
	polygon_data *listener_polygon = get_polygon_data(listener->polygon_index);
	media_data *media = listener_polygon->media_index!=NONE ? get_media_data(listener_polygon->media_index) : nullptr;
	...
	else
	{
		...
		{
			ambient_sound_image_data *image = get_ambient_sound_image_data(listener_polygon->ambient_sound_image_index);
			
			// LP change: returning NULL means this is invalid; do some editing if necessary
			if (image)
				add_one_ambient_sound_source((ambient_sound_data *)data, nullptr, listener, image->sound_index, image->volume);
			else
		...
		if (media && (media->height>=listener_polygon->floor_height || !MEDIA_SOUND_OBSTRUCTED_BY_FLOOR(media)))
		{
			...
			add_one_ambient_sound_source((ambient_sound_data *)data, &source, listener,
	...
	{
		platform_data *platform= get_platform_data(listener_polygon->permutation);
	...
	{
		map_object		*object		= 	saved_objects + index; // gross, sorry
		polygon_data	*polygon	=	get_polygon_data( object->polygon_index );
		media_data		*media		= 	not isNONE( polygon->media_index ) ? get_media_data( polygon->media_index ) : nullptr;
		
		auto sound_type = object->index;
		auto sound_volume = object->facing;
		bool active = true;
		...
		if (active && (!under_media || (media && source.point.z<media->height && polygon->media_index==listener_polygon->media_index)))
			add_one_ambient_sound_source((ambient_sound_data *)data, &source, listener, sound_type, sound_volume);
	}
...
{
	polygon_data *polygon = get_polygon_data(current_player->camera_polygon_index);
	
	if (polygon->random_sound_image_index == NONE)
		return;
	
	random_sound_image_data *image = get_random_sound_image_data(polygon->random_sound_image_index);
...
short **OriginalEnvironments = nullptr;
