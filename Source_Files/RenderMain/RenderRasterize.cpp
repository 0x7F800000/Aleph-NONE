...
void RenderRasterizerClass::render_node(sorted_node_data *node, bool SeeThruLiquids, RenderStep renderStep)
{
	...
	{
		const platform_data *restrict platform = get_platform_data(polygon->permutation);
		if (platform && PLATFORM_IS_FLOODED(platform))
		{
			auto adj_index = find_flooding_polygon(node->polygon_index);
			if (adj_index != NONE)
			{
				const polygon_data *restrict adjacent_polygon = get_polygon_data(adj_index);
	...
	if (media && !SeeThruLiquids)
	{
		...
		horizontal_surface_data *media_surface = nullptr;
	...
	for( window = node->clipping_windows; window; window = window->next_window )
	{
		...
		if (ceiling_surface.height > view->origin.z) // LP change: indicated that the void is on other side
		...
		for( i = 0; i < polygon->vertex_count; ++i )
		{
			...
			store_endpoint(
				get_endpoint_data(
				polygon->endpoint_indexes[WRAP_HIGH(i, polygon->vertex_count-1)]),
				surface.p1);
		...
		if (floor_surface.height < view->origin.z) // LP change: indicated that the void is on other side
	...
	RenderStep renderStep)
{
	...
	for (i = 0; i < vertex_count; ++i )
	{
		const flagged_world_point2d *restrict world	= vertices + 
						(adjusted_height > 0 ? vertex_count - i - 1 : i);
						
		point2d *restrict screen = textured_polygon.vertices + i;
		...
		switch (world->flags&(_clip_up|_clip_down))
		{
			case 0:
				// LP change: making it long-distance friendly
				{
					const int32 world_x 	= world->x ? world->x : 1; // fix for division by zero error
					const int32 screen_y	= view->half_screen_height - 
							transformed_height/world_x + view->dtanpitch;
...
void RenderRasterizerClass::render_node_object(render_object_data *object, bool other_side_of_media, RenderStep renderStep)
{
	
	for (const clipping_window_data *restrict window = object->clipping_windows; 
		window; window = window->next_window)
	{
							...
							long_vector2d *line,	uint16 flag)
{
	...
	do
	{
		...
		*/
		...
		if (vertex_index != first_vertex) 
			continue;
		/* we came full-circle without hitting anything */
	...
	{
		if( vertex_delta != -2 ) 
			memmove(vertices+entrance_vertex + 2, vertices + exit_vertex, (vertex_count-exit_vertex)*sizeof(flagged_world_point2d));
		vertex_delta += vertex_count;
	}
	else
	{
...
{
	//a fucking nested ternary???? thanks bungie
	bool swap = p1->y > p0->y ? false : ((p0->y==p1->y) ? (p1->x<p0->x) : true);
	
	const flagged_world_point2d *restrict local_p0		= swap ? p1 : p0;
	const flagged_world_point2d *restrict local_p1		= swap ? p0 : p1;
	
	const world_distance dx	= local_p1->x - local_p0->x;
	const world_distance dy	= local_p1->y - local_p0->y;
	...
	*/
	while( numerator <= (int32)0x3fffffff && numerator >= (int32)0xc0000000 && shift_count--) 
		numerator *= 2;
	...
	uint16 flag)
{
	...
	{
		if (vertex_delta!=-2) 
			memmove( vertices + entrance_vertex + 2, 
				 vertices + exit_vertex,
