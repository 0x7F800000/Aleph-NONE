...
void RenderSortPolyClass::sort_render_tree()
{
	...
	do
	{
		...
		else /* this is a leaf, and we can remove it from the tree */
		{
			...
			sorted_node->interior_objects = nullptr;
			sorted_node->exterior_objects = nullptr;
...
clipping_window_data *RenderSortPolyClass::build_clipping_windows(node_data *ChainBegin)
{
	...
	clipping_window_data *first_clipping_window 	= nullptr;
	clipping_window_data *last_clipping_window 	= nullptr;
	
	endpoint_clip_data *endpoint;
	line_clip_data *line;
	/* 
		ignoring what clipping parameters we've gotten, 
		this is the left and right borders of this node on the screen 
	*/
	short x0, x1; 
	ix i, j;
	...
	*/
	{
		...
		{
			auto endpoint_index = polygon->endpoint_indexes[ i ];
			
			if (TEST_RENDER_FLAG(endpoint_index, _endpoint_has_been_transformed))
			{
				auto x = endpoint_x_coordinates[endpoint_index];
	...
	for (node_data *ChainNode = ChainBegin; ChainNode; ChainNode = ChainNode->PS_Shared)
	{
		...
		for (node= ChainNode;node;node= node->parent) /* examine this node and all parents! */
		{
			...
			for( i = 0;i < node->clipping_endpoint_count; ++i )
			{
				...
				auto size = AccumulatedEndpointClips.size();
				for( j = 0; j < size; ++j )
				{
				...
				if (j!=NONE) /* if the endpoint was not a duplicate */
				{
					...
					auto Length = AccumulatedEndpointClips.size();
					AccumulatedEndpointClips.push_back(nullptr);
					if (j!=Length) 
	...
	endpoint_clip_data *left_clip = nullptr;
	endpoint_clip_data *right_clip = nullptr;
	...
	//	{
		...
		const size_t Length = ClippingWindows.size();
		POINTER_DATA OldCWPointer = POINTER_CAST( &ClippingWindows.front() );
		
		// Add a dummy object and check if the pointer got changed
		clipping_window_data Dummy;
		Dummy.next_window = nullptr;			// Fake initialization to shut up CW
		...
		if (NewCWPointer != OldCWPointer)
		{
			...
			for( size_t k = 0; k < Length; k++ )
			{
				...
				if (ClippingWindow.next_window != nullptr)
					ClippingWindow.next_window = 
					(clipping_window_data *)(NewCWPointer + (POINTER_CAST(ClippingWindow.next_window) - OldCWPointer));
			}
			for( size_t k = 0; k < SortedNodes.size(); k++)
			{
				sorted_node_data &SortedNode = SortedNodes[k];
				if (SortedNode.clipping_windows != nullptr)
					SortedNode.clipping_windows =
					(clipping_window_data *)(NewCWPointer + (POINTER_CAST(SortedNode.clipping_windows) - OldCWPointer));
			}
		}
	...
	short x1)
{
	...
	auto x = x0;
	highest_line = nullptr;
	do
	{
		locally_highest_line = nullptr;
	...
	do
	{
		locally_highest_line = nullptr; /* means lowest */
